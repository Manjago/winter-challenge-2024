...
/*
    fun GridPoint.routesToUnusedSource(limit: Int): List<List<GridPoint>> =
        bfsTo(
            from = this,
            target = { desk.isProtein(it) && !isSourceUnderHarvester(it)},
            allowedWalk = desk::isSpaceOrProtein,
            limit = limit
        )
*/
...
fun main() {
    log("gold-arena-4.2") // prev Rank 148 633 ( Rank 196 633 )
    mainLoop()
}
